#!/usr/bin/env python3
"""Build rotated-coordinate Cartesian ML samples from existing SU2 VTU results."""
...
from plot_single_su2_result import VtuData, find_field, read_vtu, triangles_from_vtu  # noqa: E402


DEFAULT_SUMMARY_CSV = PROJECT_ROOT / "datasets" / "raw" / "su2_runs" / "overnight_dataset_summary.csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "datasets" / "processed" / "overnight_rotated_npz"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "datasets" / "previews" / "processed_overnight_rotated"
...
def fixed_grid() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    return x, y, flat_points


def rotate_xy(points: np.ndarray, angle_deg: float) -> np.ndarray:
...
def coordinate_rotation_deg(original_aoa_deg: float) -> float:
    return -float(original_aoa_deg)


def rotated_flow_view(flow: VtuData, original_aoa_deg: float) -> RotatedFlowView:
    rotation_deg = coordinate_rotation_deg(original_aoa_deg)
    points = rotate_xy(np.asarray(flow.points[:, :2], dtype=np.float32), rotation_deg)
    return RotatedFlowView(
        points=points,
        point_data=flow.point_data,
        cell_types=flow.cell_types,
        cell_connectivity=flow.cell_connectivity,
    )


def read_geometry_points(geometry_name: str) -> np.ndarray:
...
def triangles_from_rotated_flow(flow: RotatedFlowView) -> np.ndarray:
    triangles = [
        cell
        for cell_type, cell in zip(flow.cell_types, flow.cell_connectivity, strict=True)
...
def interpolate_field(flow: RotatedFlowView, values: np.ndarray, flat_grid_points: np.ndarray) -> np.ndarray:
    values = np.asarray(values, dtype=np.float64).squeeze()
    triangles = triangles_from_rotated_flow(flow)
...
def get_required_field(flow: RotatedFlowView, label: str, candidates: list[str]) -> np.ndarray:
...
def write_npz(
    ...
    rotation_deg = coordinate_rotation_deg(case.aoa)
    ...
    np.savez_compressed(
        ...
        original_aoa=np.asarray(case.aoa, dtype=np.float32),
        coordinate_rotation_deg=np.asarray(rotation_deg, dtype=np.float32),
...
def save_preview(
    ...
    axes[0].set_ylabel("y")
    fig.suptitle(
        f"{case.geometry} M={mach_label(case.mach)} "
        f"(rotated, original AoA={aoa_label(case.aoa)})"
    )
    fig.tight_layout()
...
def process_case(
    ...
    rotated_flow = rotated_flow_view(flow, case.aoa)

    geometry_points = read_geometry_points(case.geometry)
    rotated_geometry = rotate_xy(geometry_points, coordinate_rotation_deg(case.aoa))
    solid_mask = create_solid_mask(rotated_geometry, flat_grid_points)

    mach_values = get_required_field(rotated_flow, "Mach", ["Mach", "Mach_Number", "MachNumber"])
    pressure_values = get_required_field(
        rotated_flow, "Pressure", ["Pressure", "Static_Pressure", "Pressure_Pa"]
    )
    density_values = get_required_field(rotated_flow, "Density", ["Density", "Rho", "rho"])
    temperature_values = get_required_field(
        rotated_flow, "Temperature", ["Temperature", "Static_Temperature"]
    )

    mach = interpolate_field(rotated_flow, mach_values, flat_grid_points)
    pressure = interpolate_field(rotated_flow, pressure_values, flat_grid_points)
    density = interpolate_field(rotated_flow, density_values, flat_grid_points)
    temperature = interpolate_field(rotated_flow, temperature_values, flat_grid_points)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build rotated-coordinate Cartesian ML samples from SU2 VTU results."
    )
    parser.add_argument("--summary", type=Path, default=DEFAULT_SUMMARY_CSV)
...
def directory_size_gib(path: Path) -> float:
    total = sum(file.stat().st_size for file in path.glob("*.npz"))
    return total / (1024**3)
...
def main() -> None:
    ...
    _x, _y, flat_grid_points = fixed_grid()
    ...
    for index, case in enumerate(cases):
        try:
            ...
            output_path = process_case(case, flat_grid_points, output_dir, preview_dir, preview_index)
    ...
    print("\nRotated Cartesian dataset build complete.")
    print(f"Summary CSV: {summary_csv}")
    print(f"Processed samples: {len(processed)}")
    print(f"Failed samples: {len(failed)}")
    print(f"Output folder: {output_dir}")
    if processed:
        print(f"Output dataset size: {directory_size_gib(output_dir):.2f} GiB")
